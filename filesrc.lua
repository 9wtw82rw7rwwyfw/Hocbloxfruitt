...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>Hocbloxfruit chào ae<Color=/>"):Display()
    wait(.5)
...
local Window = OrionLib:MakeWindow({Name = "Hocbloxfruit", HidePremium = false, IntroText = "Hocbloxfruit⚜️", SaveConfig = true, ConfigFolder = "NightHub"})
...
W:AddLabel("Hocbloxfruit SCRIPT")
W:AddLabel("DiscordHocbloxfruit")
local Section = W:AddSection({
    Name = "Status"
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "Hocbloxfruit(Noitification)",
